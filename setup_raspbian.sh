...
# check arguments
if [[ "$1" == "-h" || "$1" == "--help" ]]; then
  echo "USAGE: setup_raspbian.sh BASEDEV HOSTNAME AUTHKEYS_PATH [SSID PSK]"
...
#sed -i "s/localhost/${HOSTNAME} localhost/g" "${TMPDIR}/etc/hosts"
...
EOF
  ...
  if [[ ! -e "${TMPDIR}/etc/network/interfaces.d/ra0" ]]; then
    ra0conf=$(cat <<EOF
allow-hotplug ra0
...
EOF
    ...
    echo "Setting up /etc/network/interfaces.d/ra0"
    echo "$ra0conf" > "${TMPDIR}/etc/network/interfaces.d/ra0"
  fi
fi
